  ...
  <div>
      ...
      <el-table :data="releaseList" border style="width: 100%">
        ...
        <el-table-column fixed="right" label="操作" width="150"> 
          <template slot-scope="scope">
            ...
            <el-button @click="deleteRelease(scope.row)" type="danger" icon="el-icon-delete" size="mini" circle></el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    >
      <el-form :model="releaseForm" ref="releaseFormRef" :rules="rules">
        ...
        <el-form-item label="允许放行时间" :label-width="formLabelWidth">
        ...
        <el-form-item label="放行类型" :label-width="formLabelWidth">
          <el-input v-model="releaseForm.clearancetype" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    deleteRelease(release){
      this.$confirm(`您确认删除放行 ${release.shipid} ?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          releaseApi.deleteReleaseById(release.id).then(response => {
            this.$message({
              type: 'success',
